...
wfRICLPM <- function(varNames = varNames, 
                     sibSub = sibSub, 
                     constrained = FALSE) {
  ...
  for (i in c(sibSub))
  {
    for (j in c(names(varNames)))
    {
      RIntVar[[paste0(j, i)]] <- paste0(
        "ri_", j, i, " ~~ ", "c(v_ri_", j, ",v_ri_", j, ")*","ri_", j, i
      )
    }
  }
  ...
  for (j in sibSub)
  {
    ...
    {
      index <- paste0("c(cw_",gsub(paste0("ri_|", j), "", RIn[(i)]), 
                      gsub(paste0("ri_|", j), "",RIn[(i + 1):length(RIn)]),
                      ",cw_", gsub(paste0("ri_|", j),"", RIn[(i)]), 
                      gsub(paste0("ri_|", j), "", RIn[(i + 1):length(RIn)]), ")*"
      )
  ...
  {
    upperT[[paste0(ln[i], sibSub[1])]] <- paste0("ri_", ln[i], sibSub[1], " ~~ ", paste(paste0(
      "c(cb_",ln[i], ln[(i):length(ln)], "_mz,cb_", ln[i], ln[(i):length(ln)], 
      "_dz)*", "ri_", ln[(i):length(ln)],sibSub[2]), collapse = " + "))
  }
  ...
  {
    lowerT[[paste0(ln[i], sibSub[1])]] <- paste0("ri_", ln[i], sibSub[1], " ~~ ", paste(paste0(
      "c(cb_",ln[(i + 1):length(ln)], ln[i], "_mz,cb_", ln[(i + 1):length(ln)], 
      ln[i], "_dz)*", "ri_", ln[(i + 1):length(ln)], sibSub[2]
    ), collapse = " + "))
  }
  ...
  if (constrained == TRUE) {
    ...
    df2 <- matrix(rep(names(varNames), dimVars), 
                  nrow = dimVars, 
                  ncol = dimTraits, 
                  byrow = T) # data for constrained labels
    ...
    for (j in sibSub)
    {
      for (i in 1:dimTraits)
      {
        ...
        pred <- matrix(paste0("c(", congM, ",", congM, ")*", lagsM, j
        ), nrow = nrow(congM), byrow = F)
    ...
    for (j in sibSub)
    {
      for (i in 1:dimTraits)
      {
        ...
        lagsM <- as.matrix(lags)
        pred <- matrix(paste0(
          "c(", gsub("lat_", "", congM), ",", gsub("lat_", "", congM), ")*",
          lagsM, j
        ), nrow = nrow(congM), byrow = F)
        pred <- apply(pred, 1, paste, collapse = " + ")
        net <- rbind(net, paste0(paste0(dep_var, j), " ~ ", pred))
  ...
  if (constrained == TRUE) {
    ...
    for (i in 1:dimTraits)
    {
      ...
      congM <- as.matrix(cong)
      lagsM <- as.matrix(lags)
      pred <- matrix(paste0("c(S_", congM, ",S_", congM, ")*", lagsM, sibSub[1]
      ), nrow = nrow(congM), byrow = F)
      pred <- apply(pred, 1, paste, collapse = " + ")
    ...
    for (i in 1:dimTraits)
    {
      ...
      congM <- as.matrix(cong)
      lagsM <- as.matrix(lags)
      pred <- matrix(paste0("c(S_", congM, ",S_", congM, ")*", lagsM,sibSub[2]
      ), nrow = nrow(congM), byrow = F)
      pred <- apply(pred, 1, paste, collapse = " + ")
    ...
    for (i in 1:dimTraits)
    {
      ...
      lagsM <- as.matrix(lags)
      pred <- matrix(paste0(
        "c(S_", gsub("lat_", "", congM), ",S_", gsub("lat_", "", congM), ")*", lagsM,
        sibSub[1]
      ), nrow = nrow(congM), byrow = F)
      pred <- apply(pred, 1, paste, collapse = " + ")
      net <- rbind(net, paste0(paste0(dep_var, sibSub[2]), " ~ ", pred))
  ...
  if (constrained == TRUE) {
    ...
    for (i in sibSub)
    {
      for (j in e_v)
      {
        listE[[paste0("e_", j, i)]] <- paste0(j, i, " ~~ c(", sub("lat_", "e_", j), ",", sub(
          "lat_",
          "e_", j
        ), ")*", j, i)
      }
  ...
  if (constrained == TRUE) {
    # Contemporaneous covariances within person equal for sibs and zigosity - constrained
    ConCov <- list()
    ...
    for (S in sibSub) {
      for (i in 1:nSets) {
        ...
        {
          index <- paste0(
            "c(r_", set_c[(j + 1):nTraits], set_c[j],
            ",r_", set_c[(j + 1):nTraits], set_c[j], ")*"
          )
          ConCov <- rbind(ConCov, paste0(set[j], S, " ~~ ", paste(paste0(
            index, set[(j + 1):nTraits],
            S
          ), collapse = " + ")))
        }
      }
    ...
    # Contemporaneous covariances within person equal for sibs and zigosity - unconstrained
    ConCov <- list()
    df <- as.data.frame(latNames, stringsAsFactors = FALSE)
    nSets <- dim(df)[1]
    nTraits <- dim(df)[2]
    ...
    for (S in sibSub) {
      for (i in 1:nSets) {
        ...
        {
          index <- paste0(
            "c(", sub("lat_", "r_", set[(j + 1):nTraits]), sub("lat_", "", set[j]),
  ...
  if (constrained == TRUE) {
    # Contemporaneous covariances between siblings differ by zygosity - matrix of labels is symmetric - constrained
    UpperTCov <- list()
    df <- as.data.frame(latNames, stringsAsFactors = FALSE)
    nSets <- dim(df)[1]
    nTraits <- dim(df)[2]
    ...
    for (i in 1:nSets)
    {
      ...
      {
        index <- paste0(
          "c(Sr_", set_c[j], set_c[(j):nTraits], "_mz,Sr_",
          set_c[j], set_c[(j):nTraits], "_dz)*"
        )

        UpperTCov <- rbind(UpperTCov, paste0(set[j], sibSub[1], " ~~ ", paste(paste0(
          index, set[(j):nTraits],
          sibSub[2]
        ), collapse = " + ")))
      }
    ...
    for (i in 1:nSets)
    {
      ...
      {
        index <- paste0(
          "c(Sr_", set_c[(j + 1):nTraits], set_c[j],
    ...
    # Contemporaneous covariances between siblings differ by zygosity - matrix of labels is symmetric  - unconstrained
    UpperTCov <- list()
    df <- as.data.frame(latNames, stringsAsFactors = FALSE)
    nSets <- dim(df)[1]
    nTraits <- dim(df)[2]
    for (i in 1:nSets)
    {
      ...
      {
        index <- paste0(
          "c(", sub("lat_", "Sr_", set[j]), sub("lat_", "", set[(j):nTraits]), "_mz,",
    ...
    for (i in 1:nSets)
    {
      ...
      {
        index <- paste0(
          "c(", sub("lat_", "Sr_", set[(j + 1):nTraits]), sub("lat_", "", set[j]),
  ...
  return(list(
    ...
    RIcb_txt = RIcb_txt, 
    FI_txt = FI_txt,
    listLat_txt = listLat_txt, 
    listLagsW_txt = listLagsW_txt, 
    listLagsSib_a_txt = listLagsSib_a_txt,
    listLagsSib_b_txt = listLagsSib_b_txt, 
    listLat_e_txt = listLat_e_txt, 
    ConCov_w_txt = ConCov_w_txt,
    ConCov_s_txt = ConCov_s_txt, 
    model = model
  ))
}
